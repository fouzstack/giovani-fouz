...
import ArchiveBoxIcon from '@heroicons/react/24/outline/ArchiveBoxIcon.js';
import CubeTransparentIcon from '@heroicons/react/24/outline/CubeTransparentIcon.js';
...
// Tipos TypeScript
type SectionId = 'hero' | 'ecosystem' | 'organisms' | 'symbiosis' | 'habitat' | 'contact' | 'about';
          ...
          <div className="container mx-auto flex justify-between items-center">
                ...
                <a
                  ...
                  className={`flex items-center space-x-2 px-3 py-2 lg:px-4 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-cyan-500 ${theme === 'dark' ? 'focus:ring-offset-gray-900' : 'focus:ring-offset-white'} ${
                    activeSection === item.id
                      ? 'bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 text-cyan-400 font-semibold shadow-lg shadow-cyan-500/10'
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500 md:hidden ${theme === 'dark' ? 'focus:ring-offset-gray-900 hover:bg-gray-800/50' : 'focus:ring-offset-white hover:bg-gray-100/50'}`}
                ...
                {navItems.map((item) => (
                  <a
                    ...
                    className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${activeSection === item.id
                      ? 'bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-400 font-semibold'
                      : theme === 'dark' ? 'hover:bg-gray-800/50 text-gray-300' : 'hover:bg-gray-100/50 text-gray-700'
                    }`}
                    aria-current={activeSection === item.id ? 'page' : undefined}
                  >
          ...
          <div className="container mx-auto max-w-6xl text-center relative z-10">
            ...
            <div className="absolute bottom-20 left-1/2 -translate-x-1/2">
              <div className="flex flex-col items-center animate-bounce">
                <ChevronDownIcon className="h-6 w-6 text-cyan-400/60" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12 items-center">
              <div className="lg:col-span-1 flex flex-col items-center lg:items-start">
                <div className="relative mb-6">
                  <div className="absolute -inset-4 bg-gradient-to-r from-cyan-500/20 to-emerald-500/20 rounded-full blur-xl" aria-hidden="true"></div>
                  <div className={`relative p-6 rounded-full bg-gradient-to-br from-cyan-500/10 to-emerald-500/10 border ${theme === 'dark' ? 'border-cyan-500/30' : 'border-cyan-500/40'}`}>
                    <UserCircleIcon className="h-32 w-32 text-cyan-400" />
                  </div>
                </div>
                <h3 className={`text-2xl font-bold mb-2 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                  Giovani Fouz
                </h3>
                <p className={`text-sm mb-4 ${theme === 'dark' ? 'text-cyan-300' : 'text-cyan-600'}`}>
                  Desarrollador de Ecosistemas Híbridos
                </p>
              </div>
              ...
              <div className="lg:col-span-2">
                <div className={`p-8 rounded-2xl backdrop-blur-sm border ${theme === 'dark' ? 'bg-gradient-to-br from-gray-900/40 to-gray-900/20 border-gray-800/30' : 'bg-gradient-to-br from-white/40 to-white/20 border-gray-200/30'}`}>
                  <p className={`text-lg leading-relaxed mb-6 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                    Soy un desarrollador full-stack especializado en crear sistemas híbridos donde backend, 
                    frontend, mobile y desktop coexisten en perfecta simbiosis. Como en un arrecife de coral, 
                    creo que la verdadera resiliencia tecnológica nace de la interconexión armoniosa entre 
                    diferentes organismos tecnológicos.
                  </p>
                  <p className={`text-lg leading-relaxed mb-8 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                    Mi filosofía: cada línea de código debe ser como un pólipo de coral — aparentemente simple 
                    individualmente, pero capaz de construir estructuras masivas y resilientes cuando trabaja 
                    en simbiosis con otros.
                  </p>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <div className={`px-4 py-2 rounded-lg ${theme === 'dark' ? 'bg-gray-800/50 border border-gray-700/50' : 'bg-gray-100/50 border border-gray-200/50'}`}>
                      <span className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Full-Stack Development</span>
                    </div>
                    <div className={`px-4 py-2 rounded-lg ${theme === 'dark' ? 'bg-gray-800/50 border border-gray-700/50' : 'bg-gray-100/50 border border-gray-200/50'}`}>
                      <span className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Arquitectura Híbrida</span>
                    </div>
                    <div className={`px-4 py-2 rounded-lg ${theme === 'dark' ? 'bg-gray-800/50 border border-gray-700/50' : 'bg-gray-100/50 border border-gray-200/50'}`}>
                      <span className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Sistemas Resilientes</span>
                    </div>
                  </div>
                </div>
              </div>
